...
\section{Results}
In this section, we present the results of the mathematical model solved using the Florida case study in a vaccinated and unvaccinated scenario. We set $D=70$mi for both scenarios and our vaccine scenario parameters as $\lambda_i = 0.1$,  $q_i = 0.01$, $\omega_i=0.001$, $\ell_i=0.05$, and $V_i^0 = 0$. The model was solved using the CPLEX solver on GAMS software using all the aforementioned parameters. 
...
\begin{figure}
    ...
    \caption{Unmet Demand for Vaccinated Scenario (Left) vs. Unvaccinated Scenario (Right)}\label{fig:unmetDemand}
\end{figure}
